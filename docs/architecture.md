...
| **3 — Forecasting** | Clean-state gate, chronological split, metrics, naive baseline, Prophet, XGBoost, LSTM prep; LSTM trainer planned | `make_forecast_dataset.py`, `evaluate_forecast.py`, `train_forecast_models.py`, `build_features.create_supervised_lags`, `build_features.create_sequences` |
...
Phase 3 forecasting uses scikit-learn metrics, Prophet for the statistical baseline, XGBoost for tabular lag-based forecasting, and PyTorch for LSTM sequence prep. LSTM model training remains planned.
...
??? info "Technical deep dive"
    ...
    **Script inventory:** Phase 3 includes `verify_phase2_state.py`, `evaluate_naive_baseline.py`, `evaluate_prophet.py`, `verify_xgboost_prep.py`, `evaluate_xgboost.py`, and `verify_lstm_prep.py`.
    ...
    **Forecasting notes:** [Forecasting Baseline](forecasting-baseline.md) · [Prophet Baseline](prophet-baseline.md) · [XGBoost Prep](xgboost-prep.md) · [XGBoost Forecasting](xgboost-forecasting.md) · [LSTM Prep](lstm-prep.md) · [Phase 3 Strategy](phase3-strategy.md).
