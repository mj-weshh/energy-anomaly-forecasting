...
`main.py` is the consolidating CLI for Phase 1–3. It imports public helpers (`load_smart_meter_data`, `build_all_features`, and later model trainers) rather than duplicating pipeline logic. Day 2 covers ingest + features; anomaly detection, cleaning, and forecasting wire in subsequent E2E days. See [E2E Pipeline](e2e-pipeline.md).
...
| **3 — Forecasting** | Clean-state gate, chronological split, metrics, naive baseline, Prophet, XGBoost, LSTM, unified comparison, E2E CLI scaffold | `main.py`, `make_forecast_dataset.py`, `evaluate_forecast.py`, `train_forecast_models.py`, `lstm_model.py`, `build_features.create_supervised_lags`, `build_features.create_sequences`, `compare_forecasts.py` |
...
??? info "Technical deep dive"
    ...
    **Script inventory:** Phase 3 includes `main.py` (E2E CLI), `verify_phase2_state.py`, `evaluate_naive_baseline.py`, `evaluate_prophet.py`, `verify_xgboost_prep.py`, `evaluate_xgboost.py`, `verify_lstm_prep.py`, and `compare_forecasts.py`.
