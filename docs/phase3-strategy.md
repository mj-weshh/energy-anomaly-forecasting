...
!!! success "Executive summary"
    ...
    - **LSTM prep shipped (Week 7 Day 3):** PyTorch sliding-window sequences — see [LSTM Prep](lstm-prep.md).
    - **Terms:** [Glossary](glossary.md) — imputation, temporal split; forecasting metrics (MAE / RMSE / MAPE).

**Status:** Week 6 Day 1–3 and Week 7 Day 1–3 **complete**; LSTM trainer and research deliverables planned next  
...
### D. Deep learning (LSTM) — **prep implemented**

| | |
|--|--|
| **What** | Long Short-Term Memory network (recurrent architecture) |
| **Prep (done)** | Sliding windows → 3D tensors `[samples, time_steps, features]` via `create_sequences` — see [LSTM Prep](lstm-prep.md) |
| **Training (planned)** | PyTorch LSTM model, chronological sequence batches, test-set scoring vs naive / Prophet / XGBoost floors |
...
- LSTM model architecture, training loop, and evaluation script
...
??? info "Technical deep dive"
    ...
    **LSTM sequences (Day 3 prep):** `create_sequences` — default 24-step windows; verify with `python scripts/verify_lstm_prep.py`.

    **Metrics:** `evaluate_forecast` in `evaluate_forecast.py` (MAE / RMSE / MAPE on test for headline numbers).

    **Dependencies:** pandas, scikit-learn, `prophet>=1.1.5`, `xgboost>=2.0.0`, `torch>=2.0.0` (see `requirements.txt`). LSTM **trainer** still planned.
